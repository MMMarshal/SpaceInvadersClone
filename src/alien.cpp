...
void moveAliens(std::list<Alien>& alienList, sf::Clock& clock, int& hitWall, const float& moveDelay){
  ...
  for (i = alienList.begin(); i != alienList.end(); ++i){
    if (elapsed.asSeconds() >= moveDelay - 0.2 && i->alien.getTextureRect() == getAlienDeath())
      i = alienList.erase(i);
  }
...
bool laserCooldownComplete(const sf::Time& laserCooldown, const std::list<Alien>& aliens, const int& level){
  ...
  if (level == 3) delay = 3.5;
  if (level > 3) delay = 3.75;
  if (level > 5) delay = 3.8;
  if (level > 7) delay = 3.8;
  if (level > 9) delay = 3.9;
  if (level > 11) delay = 3.95;
...
void generateLasers(sf::Clock& laserClock, std::list<Alien>& aliens, std::list<Projectile>& laserList){
  ...
  newLaser.setPostion(sf::Vector2f(i->alien.getPosition().x + 40, i->alien.getPosition().y));
