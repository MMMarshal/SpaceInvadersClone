...
/** The Alien class contains all methods for the aliens including the alien Sprite, movement, and collision detection. Alien sprites are called from the Texture class. **/
...
/**
 Moves the entire grid of aliens. All aliens move alternate moving left and right as they hit the screen limit,
 then they drop down and move in the opposite direction.
 @param alienList - The Projectile object to check colllision with.
 @param clock - The clock used to time the alien movements.
 @param hitWall - The number of time aliens have collided with the side of the window.
 @param moveDelay - A value greater than 0 which will be used to adjust the speed of the alien movments.
 */
void moveAliens(std::list<Alien>& alienList, sf::Clock& clock, int& hitWall, const float& moveDelay);

/**
 Called when starting the game or completing a level to generated a new volly of aliens.
 @return A list containing all 50 aliens in their appropiate locations needed for a new level.
 */
...
/**
 ...
 @param clock - The clock used to time the alien movements.
 @param speed - A value greater than 0 which will be used to adjust the speed of the alien movments.
 @param moveCount - A value of 1 - 4 used to determin the one of four alien move sounds.
 */
void moveAlienSFX(sf::Clock& clock, const float& speed, int& moveCount);

/**
 Determins how quickly the alein laser cooldown takes to complete based on the level the player is on.
 @param laserCooldown - A time used to measure the last time an alien laser was fired.
 ...
 */
bool laserCooldownComplete(const sf::Time& laserCooldown, const std::list<Alien>& aliens, const int& level);

/**
 Generates a laser from a random alien on the board.
 @param laserClock - A time used to measure the last time an alien laser was fired.
 @param aliens - The list of aliens within the window.
 @param laserList - The list of all lasers within the window.
 */
void generateLasers(sf::Clock& laserClock, std::list<Alien>& aliens, std::list<Projectile>& laserList);
...
/**
 ...
 @param alienSpeedClock - A cloked used to time how long the aliens have been moving at a specific speed.
 @param speed - A value greater than 0 which will be used to adjust the speed of the alien movments.
 */
void changeAlienSpeed(sf::Time& elapsed, sf::Clock& alienSpeedClock, float& speed);
 ...
 */
void detectAlienHit(std::list<Projectile>& bulletList, std::list<Alien>& aliens, int& score);
