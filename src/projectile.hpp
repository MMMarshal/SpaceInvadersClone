...
public:
  ...
  */
  ...
  /**
   ...
   @param speed  - An int representing the direction and speed of the projectile.
   a negative value will move the projectile down the windown, and a positive
   int will move the projectile up the window.
  */
...
/**
 ...
 @param projectilesList - A list if projectiles, either bullets or lasers, that should be drawn
  on the screen.
 @param window - The game window.
 @param velocity - An int represeiting the speed and direction of the projectiles path.
 */
void drawProjectiles(std::list<Projectile>& projectilesList, sf::RenderWindow& window, const int& velocity);

/**
 Used to delete all porjectiles which have left the window.
 @param projectiles - A list if projectiles, either bullets or lasers, that are within the window
 */
